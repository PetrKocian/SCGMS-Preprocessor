...
#include "modify_filter.h"
#include "descriptor.h"

CModify_Filter::CModify_Filter(scgms::IFilter *output) : CBase_Filter(output) {
	//
}

CModify_Filter::~CModify_Filter() {
	//
}

HRESULT IfaceCalling CModify_Filter::QueryInterface(const GUID*  riid, void ** ppvObj) {
	if (Internal_Query_Interface<scgms::IFilter>(modify_filter::id, *riid, ppvObj)) return S_OK;

	return E_NOINTERFACE;
}

HRESULT IfaceCalling CModify_Filter::Do_Configure(scgms::SFilter_Configuration configuration, refcnt::Swstr_list& error_description) {
	...
	auto example_str = configuration.Read_String(modify_filter::rsExample_Config_String);	
	auto example_num = configuration.Read_Double(modify_filter::rsExample_Config_Double, 123.0); // default value parameter is optional
...
HRESULT IfaceCalling CModify_Filter::Do_Execute(scgms::UDevice_Event event) {
	...
	if (event.is_level_event() && event.signal_id() == scgms::signal_BG) {
		...
		if (event.level() > 8.0) {
			...
			evt.device_id() = modify_filter::id;
